<?php
  ...
  class Messages{
     ...
     $messages = $db->query("SELECT messages.id, messages.body, s.user_name AS sender , r.user_name AS receiver FROM messages
                 ...
                 WHERE (s.id = :friendid AND r.id = :userid) OR (s.id = :userid AND r.id = :friendid)",array(':userid' => $userid,':friendid' => $friend_id));
    ...
    public static function messageusers($userid){
          global $db;
          $messages = $db->query("SELECT r.id AS rid,s.id AS sid,s.user_name AS sender , r.profile_img ,s.profile_img , r.user_name AS receiver FROM messages
                 LEFT JOIN users s ON messages.sender = s.id
                 LEFT JOIN users r ON messages.receiver = r.id
